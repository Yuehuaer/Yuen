...
def record_coordinates():
    ...
    print("2. 按空格键记录当前位置")
    print("3. 按ESC键退出记录模式")
    print("4. 粘贴到 Excel 时，确保每组坐标单独占一单元格（如 B2 输入 267,531）")
    print("==================")
    ...
    if recorded_points:
        ...
        print("2. 粘贴后，每组坐标应单独占一单元格")
        print("3. 如果粘贴到多行，可手动调整到正确的指令行")
...
def dataCheck(sheet):
    ...
    while i < nrows:
        ...
        if cmdType in [CMD_CLICK, CMD_DOUBLE_CLICK, CMD_RIGHT_CLICK]:
            if not isinstance(cmdValue.value, str):
                print(f'第{i + 1}行,第2列: 图片路径应为字符串: {cmdValue.value}')
                checkCmd = False
        ...
        if cmdType in [CMD_COORD_CLICK, CMD_COORD_DOUBLE]:
            if not isinstance(cmdValue.value, str):
                print(f'第{i + 1}行,第2列: 坐标应为字符串格式: {cmdValue.value}')
                checkCmd = False
            else:
                # 清理坐标字符串
                coord_str = cmdValue.value.strip()
                coord_str = re.sub(r'\s+', '', coord_str)
                if not re.match(r'^-?\d+,-?\d+$', coord_str):
                    print(f'第{i + 1}行,第2列: 坐标格式错误，应为 x,y: {coord_str}')
                    checkCmd = False
        ...
        if cmdType == CMD_FALLBACK_CLICK:
            ...
            if ncols < 4 or not fallback_value or not isinstance(fallback_value.value, str):
                ...
                coord_str = fallback_value.value.strip()
                coord_str = re.sub(r'\s+', '', coord_str)
                if not re.match(r'^-?\d+,-?\d+$', coord_str):
                    print(f'第{i + 1}行,第4列: 坐标格式错误，应为 x,y: {coord_str}')
                    checkCmd = False
