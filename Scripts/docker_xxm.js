...
const DEFAULT_IMAGES = [
    ...
    // "library/nginx",  // 你可以在这里加更多，格式参考上面
    // "redis"
];

// 🚫 忽略关键词 (不监控这些 Tag)
const IGNORE_KEYWORDS = ["latest", "beta", "alpha", "test", "dev", "rc"];
...
async function checkImage(imageName) {
    $.log(`\n🔍 [${imageName}] 开始检查...`);
    const apiUrl = `https://hub.docker.com/v2/repositories/${imageName}/tags?page_size=15`;
...
function formatTime(isoString) {
    ...
    return `${nd.getMonth() + 1}-${nd.getDate()} ${nd.getHours()}:${nd.getMinutes()}`;
}
