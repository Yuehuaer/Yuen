...
!(async () => {
  ...
  // 此处通知中mediaUrl展示缩略图（这里用的就是finalUrl，建议是缩略图的URL）
  // openUrl是点击通知后打开的大图地址，和mediaUrl可以相同，也可以不同
  await notify(CONFIG.NAME, '点击查看大图', '', {
    'openUrl': finalUrl,
    'mediaUrl': finalUrl
  })
})()
...
function notify(title, subtitle, message, opts) {
  // 这里确保在 Loon 中调用的实际是 $notification.post
  // 你 Env 中的 msg 方法会自动调用正确的接口
