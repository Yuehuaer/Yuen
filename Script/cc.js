...
if ($.isNode()) {
    try {
        ...
        $.error('请安装必要的依赖: npm install node-fetch cheerio')
    }
}
// -------------------------------------
// 开发者模式
...
;(async () => {
    ...
    // 如果是Node.js环境，确保cheerio可用
    if ($.isNode()) {
        if (!cheerio) {
            try {
                $.cheerio = require('cheerio')
            } catch (e) {
                $.error('请安装cheerio依赖: npm install cheerio')
                return
            }
        } else {
            $.cheerio = cheerio
    ...
    if ($.isNode()) {
        ...
        try {
            if (notify && typeof notify.sendNotify === 'function') {
                ...
                $.log('通知功能未启用或sendNotify.js文件不存在')
            }
        } catch (e) {
            $.warn('发送通知时出错:', e.message)
        }
    }
    // 新版surge
    else if ($.isSurge()) {
